...
if sandi == 'firman12':
	print('Welcome client panpan /njangan lupa istirahat?')
else:
    ws.close()
xz = 1
status = 'tidur'
...
def on_message(ws, message):
        ...
        evn = chat['event']
        kesurupan = '{"appversion":"4.3.16","event":"live_message","token":"","useragent":"Android","message":"HAI BOSS Q KAZUYA...."}'
        if 1 == 1:
            if z == 0:
                ws.send(kesurupan)
                z = 1
...
Code Issues 0 Pull requests 0 Projects 1 Actions Wiki Security 0 Pulse Community
panpan/panpan.py
@prasnaas prasnaas Add files via upload
f57fb11 on 1 Mar
267 lines (239 sloc)  14.6 KB
